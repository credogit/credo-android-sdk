package com.impact.payment.activities;
...
import com.impact.payment.adapters.CustomAdapter;
import com.impact.payment.adapters.CustomAdapterCardType;
        ...
        transactions.thirdPartyPay(amount, orderCurrency, cn, em, ey, cv, transRef, customerEmail, customerName, customerPhone, paymentSlug, new Transactions.ThirdPartyCallBack() {
            ...
            public void onSuccess(ThirdPartySchema schema) {
                if(schema != null && schema.getStatus() != null && schema.getStatus().equalsIgnoreCase("success")){
                    loadingV(false);
                    startActivity(new Intent(MainActivity.this, SuccessActivity.class).putExtra("status", "success"));
                    finish();
                }else{
                    loadingV(false);
                    startActivity(new Intent(MainActivity.this, SuccessActivity.class).putExtra("status", "failed"));
                    finish();
                }
            }
            ...
            public void onFailure(Throwable t) {
                loadingV(false);
                startActivity(new Intent(MainActivity.this, SuccessActivity.class).putExtra("status", "network"));
                finish();
            }
        });
    }
            ...
            transactions.verifyCardNumber(cn, orderCurrency, paymentSlug, new Transactions.VerifyCardCallBack() {
                ...
                public void onSuccess(VerifyCardSchema schema) {
                    if(schema != null && schema.getGatewayRecommendation() != null && schema.getGatewayRecommendation().equalsIgnoreCase("PROCEED")){
                        ...
                        new AlertDialog.Builder(MainActivity.this).setMessage("Invalid card number. please try again")
                ...
                @Override
                public void onFailure(Throwable t) {
                    new AlertDialog.Builder(MainActivity.this).setMessage("a network error occurred during request. try again")
                            .setPositiveButton("dismiss", new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialog, int which) {
                                    dialog.dismiss();
        ...
        transactions.initiatePayment(amount, orderCurrency, "https://google.com", "1234567890", "CARD", customerName, customerEmail, customerPhone, new Transactions.InitiatePaymentCallBack(){
            ...
            public void onSuccess(InitiateSchema schema) {
                if(schema != null && schema.getPaymentSlug() != null && !schema.getPaymentSlug().isEmpty()){
                    ...
                    Toast.makeText(MainActivity.this, "Initiating payment success", Toast.LENGTH_SHORT).show();
                    loadingV(false);
                }else{
                    loadingV(false);
                    Toast.makeText(MainActivity.this, "Initiating payment failed", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Throwable t) {
                Toast.makeText(MainActivity.this, "oops! a network error terminated request", Toast.LENGTH_SHORT).show();
                loadingV(false);
            }
        });
